...
x <- c(47.81, 47.81, 47.81, 46.09, 46.09, 46.09, 40.09, 40.09, 40.09)
y <- c(12.87, 12.87, 12.87, 14.29, 14.29, 14.29, 22.44, 22.44, 22.44)
geo$x <- x
geo$y <- y
Dgeo <- dist(geo)
...
image(dens, col=transp(myPal(30),.7), add=TRUE) 
abline(lm(Dgen~Dgeo)) 
title("Isolation by distance plot")
```


```{r ibd_2, eval=FALSE, include=FALSE}
# Dgen <- dist.genpop(obj_pop,method=2) #maybe only with genepop object
Dgen <- dist(obj$tab)
# theoretically any genetic distance can be used
